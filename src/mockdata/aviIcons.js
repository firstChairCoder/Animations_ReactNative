// Grabbed from https://www.flaticon.com/packs/gamer-avatars
// copy(JSON.stringify([...document.querySelectorAll('.icon--holder img')].slice(0, 10).map(x => x.src), null, 2))

export default [
  "https://cdn-icons-png.flaticon.com/512/4331/4331014.png",
  "https://cdn-icons-png.flaticon.com/512/4333/4333607.png",
  "https://cdn-icons-png.flaticon.com/512/4333/4333609.png",
  "https://cdn-icons-png.flaticon.com/512/4333/4333613.png",
  "https://cdn-icons-png.flaticon.com/512/4333/4333614.png",
  "https://cdn-icons-png.flaticon.com/512/4333/4333617.png",
  "https://cdn-icons-png.flaticon.com/512/4333/4333620.png",
  "https://cdn-icons-png.flaticon.com/512/4333/4333623.png",
  "https://cdn-icons-png.flaticon.com/512/4333/4333626.png",
  "https://cdn-icons-png.flaticon.com/512/4333/4333629.png"
];
