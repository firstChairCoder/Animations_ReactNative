...
const Button = ({ item, index, activeY, offset }: ButtonProps) => {
	...
	const isItemActive = useDerivedValue(() => {
		...
		const isValid =
			pressedPoint >= itemStartPos && pressedPoint < itemEndPos;
	...
	const animatedStyle = useAnimatedStyle(() => {
		...
		const isItemOutOfView =
			itemEndPos < offset.value ||
			itemStartPos > offset.value + TOOLBAR_HEIGHT;
		...
		return {
			...
			top:
				offset.value < 0 // is Top?
					...
					? -(24 - index + 1) *
					  Math.abs((offset.value - scrollValidLimit) / 10)
					: 0,
